...
@Suite
struct Extensions_FilePathAndURL_Tests {
    ...
    @Test
    func canonicalize_notPartial() throws {
    ...
    #if os(macOS)
    @Test
    func canonicalizeIfPossible_partial() {
        let uniqueName = "\(UUID().uuidString)"
    ...
    @available(macOS 12.0, *)
    @Test
    func canonicalized_notPartial() throws {
        // write temp file including a mix of uppercase and lowercase letters
        let fileURL = FileManager.default
            .temporaryDirectory
            .appendingPathComponent("\(UUID().uuidString)-TeSt123AbC.txt")
    ...
    #if os(macOS)
    @Test
    func canonicalizedIfPossible_partial() {
        let uniqueName = "\(UUID().uuidString)"
